...
# # Load book dataset from a local CSV file
# def load_local_dataset(): 
#     file_path = "book_data.csv"  # Ensure this CSV file is present in the project folder
#     df = pd.read_csv(file_path)
...
# # Load books from local dataset
# df = load_local_dataset()

# # Combine all relevant text features into a single column
# df["combined_text"] = df["Title"] + " " + df["Authors"] + " " + df["Description"] + " " + df["Category"] + " " + df["Publisher"]

# # Compute embeddings for all book features
# book_embeddings = model.encode(df['combined_text'].tolist(), convert_to_tensor=True)
...
#     # Compute cosine similarity between user query and book descriptions
#     similarities = cosine_similarity(query_embedding.cpu().numpy(), book_embeddings.cpu().numpy())[0]
    
#     # Rank books by similarity score
#     df["similarity"] = similarities
#     top_books = df.sort_values(by="similarity", ascending=False).head(3)  # Top 3 recommendations
